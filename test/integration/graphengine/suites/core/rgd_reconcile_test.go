...
//
// RGD: namespaced Kind=WebApp with spec.name string
...
//   - ApplyResult.Applied carries both managed resources
func TestRGDReconcileApply(t *testing.T) {
	env := environment.Shared(t)
	ns := env.CreateNamespace(t)

	// ── 1. Build and ensure the WebApp CRD ──────────────────────────────────
	rgd := newWebAppRGD("webapp-f3a", "WebApp", ns)
	...
	instance.SetGroupVersionKind(schema.GroupVersionKind{
		Group:   webAppGVR.Group,
		Version: webAppGVR.Version,
		Kind:    "WebApp",
	})
	...
	t.Cleanup(func() {
		_ = env.Client.Delete(env.Ctx, instance)
	})

	// Re-fetch to get the server-populated fields (UID, resourceVersion, etc.)
	if err := env.Client.Get(env.Ctx, types.NamespacedName{Name: "demo-app", Namespace: ns}, instance); err != nil {
	...
	// The compiler is accessible through the environment's Cfg; build one.
	httpClient, err := restHTTPClientFor(t, env)
	if err != nil {
		t.Fatalf("build http client: %v", err)
	}
	comp, err := compilerFromEnv(t, env, httpClient)
	if err != nil {
...
// restHTTPClientFor is a thin shim so the test does not import rest directly.
func restHTTPClientFor(t *testing.T, env *environment.Env) (*http.Client, error) {
	t.Helper()
	return rest.HTTPClientFor(env.Cfg)
}

// compilerFromEnv builds a fresh compiler.Compiler from the envtest config.
func compilerFromEnv(t *testing.T, env *environment.Env, httpClient *http.Client) (*compiler.Compiler, error) {
	t.Helper()
	return compiler.NewCompiler(env.Cfg, httpClient)
}
...
// Ensure kroclient import is used (the crdClient helper is in rgd_crd_serve_test.go).
var _ kroclient.CRDClient = nil
