...
func (ctx *CompilationContext) buildDependencyGraph(nodes map[string]*Node, inspector *ast.Inspector) (*dag.DirectedAcyclicGraph[string], []string, error) {
	...
	var captured []string
	for _, n := range nodes {
		capt, err := ctx.analyzeNodeRefs(n, nodes, inspector)
		if err != nil {
...
func (ctx *CompilationContext) analyzeNodeRefs(n *Node, nodes map[string]*Node, inspector *ast.Inspector) ([]string, error) {
	var captured []string
	for _, analyze := range []func(*Node, map[string]*Node, *ast.Inspector) ([]string, error){
		ctx.analyzeVariables,
		ctx.analyzeForEach,
		ctx.analyzeIncludeWhen,
		ctx.analyzeReadyWhen,
	} {
		capt, err := analyze(n, nodes, inspector)
		if err != nil {
			return nil, err
		}
		captured = append(captured, capt...)
...
func (ctx *CompilationContext) analyzeVariables(n *Node, nodes map[string]*Node, inspector *ast.Inspector) ([]string, error) {
...
func (ctx *CompilationContext) analyzeForEach(n *Node, nodes map[string]*Node, inspector *ast.Inspector) ([]string, error) {
	iteratorNames := nodeIteratorNames(n)
	var captured []string
