...
func TestCompile(t *testing.T) {
	...
	cases := []struct {
		...
		{
			// A reference in an expression — including optional chaining — is
			// always a hard dependency: adds a DAG edge and enforces ordering.
			name: "optional reference is hard dependency",
			graph: generator.NewGraph("g",
				generator.WithTemplate("cm", map[string]any{
					"apiVersion": "v1", "kind": "ConfigMap",
			...
			after: func(t *testing.T, prog *Program, _ *expv1alpha1.Graph) {
				cm := prog.Nodes["cm"]
				assert.Equal(t, []string{"later"}, cm.HardDepIDs(), "optional ref must be a hard dependency")
				assert.Empty(t, cm.SoftDepIDs(), "optional ref must not be a soft dep")
				assert.Contains(t, prog.DAG.Vertices["cm"].DependsOn, "later", "optional ref must add a DAG edge")
				assert.Less(t, indexOf(prog.TopologicalOrder, "later"), indexOf(prog.TopologicalOrder, "cm"))
			},
		},
		{
...
func TestCompile_WithSoftDependencies(t *testing.T) {
	...
	g := generator.NewGraph("g",
		generator.WithTemplate("cm", map[string]any{
			"apiVersion": "v1", "kind": "ConfigMap",
			"metadata": map[string]any{"name": "cfg"},
			"data":     map[string]any{"k": "${later.field}"},
		}),
		generator.WithDef("later", map[string]any{"field": "v"}),
	)
	prog, err := newTestCompiler(t).CompileWithOptions(g, WithSoftDependencies("cm"))
	require.NoError(t, err)
	cm := prog.Nodes["cm"]
...
func TestCompile_WithCostLimit(t *testing.T) {
	...
	t.Run("default unlimited allows standard compile", func(t *testing.T) {
		t.Parallel()
		g := generator.NewGraph("g",
			generator.WithNamespace("default"),
			generator.WithTemplate("cm", map[string]any{
	...
	t.Run("setting cost limit configures compiler", func(t *testing.T) {
		t.Parallel()
		c := newTestCompiler(t).WithCostLimit(5000)
		assert.Equal(t, uint64(5000), c.costLimit)
	})
}
